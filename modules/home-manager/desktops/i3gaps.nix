  ...
  config = mkIf cfg.enable {
    ...
    in {
      ...
      config =
        ...
        // {
          keybinds."${common.config.modifier}+Shift+e" = "exec i3-nagbar -t warning -m 'Do you want to exit i3?' -b 'Yes' 'i3-msg exit'";
          keybinds."Print" = "exec --no-startup-id \"$HOME/.local/bin/screenshot.sh\"";
          startup =
            map (cmd: {command = cmd;})
            (
